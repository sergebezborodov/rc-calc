...
class Y
{
    ...
    /**
     * Выводит модель как массив
     *
     * @static
     * @param mixed $var
     */
    public static function dar($var) {
        if (is_array($var)) {
            foreach ($var as $i) {
                self::dump($i->attributes, false);
            }
            Yii::app()->end();
        }
        self::dump($var->attributes);
    }
    ...
    /**
     * Возвращает CDbCommand
     *
     * @static
     * @param int $cacheLife время жизни кеша
     * @return CDbCommand
     */
    public static function dbc($cacheLife = 0) 
    {
        return Yii::app()->db->cache($cacheLife)->createCommand();
    }
    ...
    {
        return Yii::app()->getComponent('mongo')->getDb();
    }
}
